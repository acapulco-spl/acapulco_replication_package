...
class DeXor extends FADPrincipleTester {
		...
		if (f.parentFeature.isXORGroup) {
			...
			if (!alwaysActiveFeatures.contains(f.parentFeature)) {
				...
				if (siblings.size == 1) {
					...
					assertTrue('''Must have an activation of the single sibling of «f.name».''', consequences.exists[activationOf(siblings.head)])
				} else {
