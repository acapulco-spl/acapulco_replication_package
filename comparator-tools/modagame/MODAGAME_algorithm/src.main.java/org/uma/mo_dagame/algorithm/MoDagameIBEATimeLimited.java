 ...
 */
public class MoDagameIBEATimeLimited extends AbstractMoDagameIBEA {

  
	private long stoppingValue;
	private ToolInstrumenter toolInstrumenter;

	public MoDagameIBEATimeLimited(MoFmProblem problem, long stoppingValue, ToolInstrumenter toolInstrumenter) {
		super(problem);
       ...
       // this.toolInstrumenter.collectStep(evaluations, archive, solutionSet);
        ...
        while (elapsed < this.stoppingValue) {
            ...
            while (offSpringSolutionSet.size() < populationSize) {
                ...
                } while (j < MoDagameIBEATimeLimited.TOURNAMENTS_ROUNDS); // do-while
                int k = 0;
                do {
                    k++;
                    parents[1] = (Solution) selectionOperator.execute(archive);
                } while (k < MoDagameIBEATimeLimited.TOURNAMENTS_ROUNDS); // do-while
            ...
            //System.out.println("TOOLS_INSTRUMENTER TIME> " + this.toolInstrumenter);
            if (evaluations % populationSize == 0) {
            	this.toolInstrumenter.collectStep(evaluations, archive, offSpringSolutionSet);
            }
